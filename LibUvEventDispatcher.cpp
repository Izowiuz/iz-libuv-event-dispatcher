...
i9ms::LibUvEventDispatcher::LibUvEventDispatcher(QObject* parent)
...
i9ms::LibUvEventDispatcher::~LibUvEventDispatcher()
{
...
uv_loop_t* i9ms::LibUvEventDispatcher::rawLoop() const
{
    return m_uvLoop;
}

void i9ms::LibUvEventDispatcher::interrupt()
{
    m_interrupt = true;
}

bool i9ms::LibUvEventDispatcher::processEvents(QEventLoop::ProcessEventsFlags flags)
{
    // we are awake!
    emit awake();
...
void i9ms::LibUvEventDispatcher::registerSocketNotifier(QSocketNotifier* notifier)
{
...
void i9ms::LibUvEventDispatcher::socketCallback(uv_poll_s* w, int /*status*/, int events)
{
...
void i9ms::LibUvEventDispatcher::unregisterSocketNotifier(QSocketNotifier* notifier)
{
    // transform QSocketNotifier::Type to uv_poll_event
    int events = qtouv(notifier->type());
    if (events == -1) {
    ...
    if (pollerData->events == 0) {
        ...
        ::uv_close(( uv_handle_t* )pollerData->uvPollData, [](uv_handle_t* handle) {
            delete handle;
        });
    } else {
...
void i9ms::LibUvEventDispatcher::enableSocketNotifier(QSocketNotifier* notifier)
{
    // transform QSocketNotifier::Type to uv_poll_event
    int events = qtouv(notifier->type());
    if (events == -1) {
...
void i9ms::LibUvEventDispatcher::disableSocketNotifier(QSocketNotifier* notifier)
{
    int events = qtouv(notifier->type());
    if (events == -1) {
        return;
...
int i9ms::LibUvEventDispatcher::remainingTime(int timerId)
{
...
void i9ms::LibUvEventDispatcher::registerTimer(int timerId, int interval, Qt::TimerType timerType, QObject* object)
{
    ...
    if (it == m_objectTimerIndex.end()) {
        m_objectTimerIndex.emplace(std::make_pair(object, std::unordered_set<TimerData*>{ timerData }));
    } else {
        it->second.insert(timerData);
    }

    m_timers.emplace(std::make_pair(timerId, timerData));

    ::uv_timer_start(timer, LibUvEventDispatcher::timerCallback, interval, interval);
}

bool i9ms::LibUvEventDispatcher::unregisterTimer(int timerId)
{
    auto it = m_timers.find(timerId);
    if (it != m_timers.end()) {
...
QList<QAbstractEventDispatcher::TimerInfo> i9ms::LibUvEventDispatcher::registeredTimers(QObject* object) const
{
    ...
    if (it != m_objectTimerIndex.end()) {
        ...
        for (const auto timerData : it->second) {
            res.append({ timerData->timerID, timerData->interval, timerData->timerType });
        }
    }
...
bool i9ms::LibUvEventDispatcher::unregisterTimers(QObject* object)
{
    auto it = m_objectTimerIndex.find(object);
    if (it != m_objectTimerIndex.end()) {
...
void i9ms::LibUvEventDispatcher::wakeUp()
{
    ::uv_async_send(m_wakeupHandle);
}

void i9ms::LibUvEventDispatcher::flush()
{
    qCritical("LibUvEventDispatcher::flush() called.");
}

bool i9ms::LibUvEventDispatcher::hasPendingEvents()
{
    qFatal("LibUvEventDispatcher::hasPendingEvents() called.");
    return false;
}

int i9ms::LibUvEventDispatcher::qtouv(QSocketNotifier::Type qtEventType) const
{
    switch (qtEventType) {
...
void i9ms::LibUvEventDispatcher::timerCallback(uv_timer_s* w)
{
...
void i9ms::LibUvEventDispatcher::timerDeleteCallback(uv_handle_s* w)
{
    uv_timer_s* timer = ( uv_timer_s* )w;
...
bool i9ms::LibUvEventDispatcher::registerEventNotifier(QWinEventNotifier* notifier)
{
    Q_UNUSED(notifier)
    qFatal("LibUvEventDispatcher::registerEventNotifier() called.");
    return false;
}

void i9ms::LibUvEventDispatcher::unregisterEventNotifier(QWinEventNotifier* notifier)
{
    Q_UNUSED(notifier)
    qFatal("LibUvEventDispatcher::unregisterEventNotifier() called.");
}
